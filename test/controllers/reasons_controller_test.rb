...
class ReasonsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create reason" do
    assert_difference("Reason.count") do
      post reasons_url, params: { reason: { name: @reason.name, position: @reason.position, user_id: @reason.user_id } }
    end
  ...
  test "should update reason" do
    patch reason_url(@reason), params: { reason: { name: @reason.name, position: @reason.position, user_id: @reason.user_id } }
